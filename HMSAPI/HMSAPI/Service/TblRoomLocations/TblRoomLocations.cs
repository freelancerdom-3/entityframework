    ...
    public class TblRoomLocations : ITblRoomLocations
    {
        ...
        public async Task<APIResponseModel> GetAll(string? searchBy = null)
        {
            ...
            try
            {
                ...
                inner join TblRoom on TblRoom.RoomID = TblRoomLocations.RoomID
                  and TblRoom.IsActive = 1 
                    where TblRoomLocations.IsActive = 1
                             and
                     TblRoom.RoomID like '%{searchBy}%'").ToListAsync();
        ...
        public async Task<APIResponseModel> GetByID(int id)
        {
            ...
            try
            {
                using (var connection = _hsmDbContext)
                {
                    data = await connection.TblRoomLocations.Where(x => x.RoomLocationID == id && x.IsActive == true)
                        .FirstAsync();
                    if (data != null)
                    {
                        responseModel.StatusCode = HttpStatusCode.OK;
